...
stp_err_4 = []
stp_err_5 = []
STP_err_list = [stp_err_1, stp_err_2, stp_err_3, stp_err_4, stp_err_5]
gld_err_1 = []
gld_err_2 = []
gld_err_3 = []
gld_err_4 = []
...
for problem in probs_under_100:
    for i in range(5):
        ...
        min1 = run_STP_pycutest(p_invoke_, copy.copy(x0_invoke_), function_budget_)
        STP_err_list[i].append(min1)
        print('\n')
        # GLD.
...
"""
# average STP.
arrays_stp = [np.array(x) for x in STP_err_list]
STP_average_error = [np.mean(k) for k in zip(*arrays_stp)]
# average GLD.
...
list_of_errors = [STP_average_error, GLD_average_error, SignOPT_average_error, SCOBO_average_error, CMA_average_error]
list_of_algorithms = ['STP', 'GLD', 'SignOPT', 'SCOBO', 'CMA']
...
path_name = "/Users/ishaslavin/Downloads/oct25_research_AVERAGE_3_CHART_all5algorithms_DF.csv"
df.to_csv(path_name)
